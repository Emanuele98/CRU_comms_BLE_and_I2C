...
#define DEVICE_NAME                     "Airfuel PRU"                          /**< Name of device. Will be included in the advertising data. */
...
{      
		...
		if (m_dyn_payload.temp_ratio > OVER_TEMPERATURE)
		{	m_alert_payload.alert_field.overtemperature = 1;	}
		else
		{	m_alert_payload.alert_field.overtemperature = 0;	}

        // Validate voltage levels
		if (m_dyn_payload.vrect > OVER_VOLTAGE)
		{	m_alert_payload.alert_field.overvoltage = 1;	}
		else
		{	m_alert_payload.alert_field.overvoltage = 0;	}

        // Validate current levels
		if (m_dyn_payload.irect > OVER_CURRENT)
		{	m_alert_payload.alert_field.overcurrent = 1;	}
		else
		{	m_alert_payload.alert_field.overcurrent = 0;	}


        // Values are then assigned to global payload instance of dynamic characteristic
        m_dyn_payload.pru_alert = m_alert_payload.alert_field.internal;
...
void app_main(void)
{
    ...
    rc = ble_svc_gap_device_name_set("Airfuel PRU");
    assert(rc == 0);

    /* Host management loop (Host context) */
    nimble_port_freertos_init(host_task);

    /* Initialize all elements of PRU (timers and ADCs)*/
    init_setup();
